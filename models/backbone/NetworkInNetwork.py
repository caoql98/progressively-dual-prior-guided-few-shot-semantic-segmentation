...
def NNet(size=321, pretrained=True):
    opt = {'num_classes':4, 'num_stages': 4}
    net = create_model(opt)
    if pretrained:
        snapshot = '/disk2/caoqinglong/CIFAR10_RotNet_NIN4blocks/CIFAR10_RotNet_NIN4blocks/model_net_epoch200.pth'
        checkpoint = torch.load(snapshot)
        a,b = checkpoint.items()
        net.load_state_dict(b[1])
        net.train()
    return net
