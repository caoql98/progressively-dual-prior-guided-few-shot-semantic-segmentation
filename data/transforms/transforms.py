...
__all__ = ["Compose", "ToTensor", "ToPILImage", "Normalize", "Resize", "Scale", "CenterCrop", "Pad",
           ...
           "Grayscale", "RandomGrayscale","RandScale","RandRotate","RandomGaussianBlur","Crop_aug","RandomHorizontalFlip2",
           "Resize2"]
...
class ToPILImage(object):
    ...
    def __call__(self, pic, mask):
        ...
        return pic, F.to_pil_image(mask, self.mode)
...
class Resize(object):
    ...
    def __init__(self, size, interpolation=Image.BILINEAR):
        ...
        self.size = size
        self.interpolation_img = interpolation
        self.interpolation_mask = Image.NEAREST
    ...
    def __call__(self, img, mask):
        ...
        """
        return F.resize(img, self.size, self.interpolation_img), F.resize(mask, self.size, self.interpolation_mask)
...
class RandomResizedCrop(object):
    ...
    def __init__(self, size, scale=(0.08, 1.0), ratio=(3. / 4., 4. / 3.), interpolation=Image.BILINEAR):
        ...
        self.interpolation2 = Image.NEAREST
        self.scale = scale
        self.ratio = ratio
    ...
    def __call__(self, img, mask):
        ...
        img1 = F.resized_crop(img, i, j, h, w, self.size, self.interpolation)
        mask1 =  F.resized_crop(mask, i, j, h, w, self.size, self.interpolation2)
        area = mask1.size[0] * mask1.size[1]
        if np.asarray(mask1).sum()==0:
            # img1 = F.resized_crop(img, 0, 0, img.size[0], img.size[1], self.size, self.interpolation)
            # mask1 =  F.resized_crop(mask, 0, 0, mask.size[0], mask.size[1], self.size, self.interpolation2)
            img1 = F.resize(img,self.size,self.interpolation)
            mask1 = F.resize(mask,self.size,self.interpolation2)
        if np.asarray(mask1).sum()== area:
            # img1 = F.resized_crop(img, 0, 0, img.size[0], img.size[1], self.size, self.interpolation)
            # mask1 =  F.resized_crop(mask, 0, 0, mask.size[0], mask.size[1], self.size, self.interpolation2)
            img1 = F.resize(img,self.size,self.interpolation)
            mask1 = F.resize(mask,self.size,self.interpolation2)
