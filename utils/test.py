    ...
    for group in range(0,1):
        ...
        for data in val_dataloader:
            ...
            with torch.no_grad():
                ...
                # loss_val,loss_part1, loss_part2, pred = model(query_img, support_img, support_mask,query_mask)
                query_img = F.upsample(query_img, size=(size[0], size[1]), mode='bilinear')
                # b,h,w = query_mask.shape
                # query_mask = query_mask.view(b,1,h,w).float()
                # query_mask[query_mask>1]=0
                query_mask = F.upsample(query_mask, size=(size[0], size[1]), mode='nearest')
                # pred = pred.unsqueeze(0).float()
                # print(pred.shape)
                # pred = F.upsample(pred, size=(size[0], size[1]), mode='bilinear')
